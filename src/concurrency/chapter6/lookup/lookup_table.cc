...
private:
  class bucket_type {
    friend class threadsafe_lookup_table<Key, Value, Hash>;
  private:
...
int main() {
  threadsafe_lookup_table<int, std::string> ids;
  ids.add_or_update_mapping(0, "hello");
  ids.add_or_update_mapping(1, "world");
  std::cout << ids.value_for(1, "") << std::endl;
  ids.remove_mapping(0);

  // snapshot
  std::map<int, std::string> m = ids.get_map();
  for (auto &k:m) {
