    ...
    for (int i = 0; i < number; ++i) {
      ...
      if (sockfd == listenfd) {
        ...
        if (connfd >= 0) {
          ...
          auto timer = std::make_shared<Timer>(3 * TIMESLOT);
        ...
        if (ret <= 0) {
          ...
          if (timer) {
            // delete then reinsert a new one
            timer_heap.del_timer(timer);

            auto new_timer = std::make_shared<Timer>(3 * TIMESLOT);
            new_timer->user_data = &user_data[sockfd];
            new_timer->cb_func = callback_func;
