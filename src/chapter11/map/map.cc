...
multiset<Sales_data, SDComp>
        bookstore2([](const Sales_data &l, const Sales_data &r) { return l.isbn() < r.isbn(); });
...
int main() {
    ...
    if (!insert_ret.second) {           // Anna was already in the map
    ...
    cout << " " << map_it->second << endl; // prints the value of the element
    ++map_it->second;              // ok: we can change the value through an iterator
    cout << word_count["test"] << endl;
    return 0;
}
